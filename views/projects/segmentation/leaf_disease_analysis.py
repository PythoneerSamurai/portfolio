...
st.write("""
In this project, I have utilized semantic segmentation to segment leaf diseases. I have successfully
trained a model that can perform the task mentioned earlier with high precision and accuracy.
""")
...
st.write("""
In the case of this project, only one yolov8x model has been utilized, for leaf disease 
segmentation.
...
provided by "**Fakhre Alam**" on Kaggle.

The dataset has only one segmentation class, "disease".
...
st.write("##### Model Used")
st.write("""
As mentioned above, only one model has been used in this project: a **yolov8x** model for leaf disease
...
is defined to handle all functionality, ranging from inference conversion (from YOLO format to
