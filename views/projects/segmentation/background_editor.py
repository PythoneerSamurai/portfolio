...
st.write("""
In this project I have utilized semantic segmentation for the purpose of image or video background editing. I have
...
st.write("""
In the case of this project, only one yolov9e model has been utilized, that for the purpose of person segmentation.

Multiple datasets were used for the purpose of training the model. I had placed a lot of effort in finding the best
freely available person segmentation datasets that had almost pixel-perfect annotations. The datasets used fpr training
are listed below.
...
st.write("##### Model Used")
st.write("""
As mentioned above, only one model has been used in this project, that is a **yolov9e** model for person segmentation. 
...
In the case of this project, pretrained model (yolov9e.pt) was not used, rather a new model (yolov9e.yaml) was trained 
...
st.write("""
In this part of the prediction pipeline, various variables are declared for the purpose of storing the paths to the
...
Afterwards, the trained yolov9e model for person detection is instantiated using the ultralytics.YOLO class, accessed
through the "**ultralytics**" Python package.
""")

st.write("##### Defining Functions for Background Editing")
st.write("""
In this part of the prediction pipeline, two functions are defined for the purpose of background editing, one for images
