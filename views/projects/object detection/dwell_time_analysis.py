...
st.write("""
In the case of this project, only one yolov10x model has been utilized, that for the purpose of customer head detection.
...
st.write("##### Model Used")
st.write("""
As mentioned above, only one model has been used in this project, that is a **yolov10x** model for customer head 
...
In the case of this project, pretrained model (yolov10x.pt) was not used, rather a new model (yolov10x.yaml) was 
...
Following formula is used for time estimation:
""")
...
st.write("""
In this part of the prediction pipeline, the input video is loaded in to memory using the cv2.VideoCapture class. The
...
1) LabelAnnotator is used to annotate the frames with the the time each customer spends in the cashier counter queues.
...
st.write("""
In this part of the prediction pipeline, a tracker (sv.ByteTrack) is initialized for the purpose of tracking the 
customers throughout the video frames. This is important to keep track of the total time spent by the customers in the 
cashier counter queues. In addition to the tracker, a cv2.VideoWriter object is also initialized for the purpose of 
merging the annotated frames into the output video.
""")
